...
class Realtime_Sizeball_MBR_7Day_Ago extends React.Component {
  constructor(props) {
    super(props);
  ...
  Click_value_ball_size = async () => {
    ...
    if (this.state.process === "MA") {
      ...
      const array = await httpClient.post(
        server.realtime_MBRC_Ball_Size_MA_7DAY_ACCUM +
          "/" +
      ...
      const array = await httpClient.post(
        server.realtime_MBRC_Ball_Size_MD_7DAY_ACCUM +
          "/" +
          this.state.start_date ,
    ...
    try {
      if (this.state.data_tableDaily !== null) {
        ...
        return this.state.data_tableDaily.map((item) => (
          <tr>
            <td>{item.mfg_date}</td>
            <td>{item.sum_totalSize10}</td>
            <td>{item.sum_totalSize20}</td>
            <td>{item.sum_totalSize30}</td>
            <td>{item.sum_totalSize40}</td>
            <td>{item.sum_totalSize50}</td>
          </tr>
        ));
      }
    } catch (error) {}
          ...
          <div className="container-fluid">
            <div className="row-12">
              <div className="card">
                ...
                >
                Accumulate : Ball usage (%) By 7 Day Ago
                </div>
...
export default Realtime_Sizeball_MBR_7Day_Ago;
